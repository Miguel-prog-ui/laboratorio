...
if (isset($_POST['submit'])) {
    ...
    if (!empty($errors)) {
        ...
        $sql = "INSERT INTO direcciones (Tratamiento, Nombre, Apellido, Calle, CP, Localidad, Tel, Movil, Mail, Website, Categoria, Notas) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $dp->prepare($sql);

        if ($stmt === false) {
            die("<p>Error en la preparación de la consulta: " . htmlspecialchars($dp->error) . "</p>");
        }
        ...
        if ($stmt->execute()) {
            ...
            echo "<p>Datos <b>no</b> agregados: " . htmlspecialchars($stmt->error) . "</p>";
        }
    ...
    if ($resultado2 === false) {
        die("<p>Error en la consulta de categorías: " . htmlspecialchars($dp->error) . "</p>");
    }
